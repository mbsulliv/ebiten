...
func (u *UserInterface) loopGame() error {
	for {
		...
		if _, _, err := u.context.updateFrame(u.graphicsDriver, float64(C.kScreenWidth), float64(C.kScreenHeight), int(C.kScreenWidth), int(C.kScreenHeight), theMonitor.DeviceScaleFactor(), u, true); err != nil {
			return err
		}
	}
}
